...
def generate_model_doc(
    model_name: str, model_id: str, inference_dir: Path, docs_dir: Path, local_transformers_path: Optional[Path] = None
):
    ...
    if classes:
        for class_name in classes:
            doc_content.extend(["", f"## {class_name}", "", f"[[autodoc]] models.inference.{model_name}.{class_name}"])
    else:
    ...
    for section in toc[0]["sections"]:
        if section.get("title") == "Models and Pipelines Inference API":
            ...
            if not transformers_section:
                transformers_section = {"title": "Transformers Models", "sections": []}
                for category in ["TEXT MODELS", "VISION MODELS", "AUDIO MODELS", "MULTIMODAL MODELS"]:
                    category_section = {"title": category, "sections": []}
                    transformers_section["sections"].append({"title": category, "sections": [], "isExpanded": False})
                section["sections"].insert(0, transformers_section)

            model_entry = {"local": f"model_doc/transformers/{model_name}", "title": model_title}
...
def main():
    ...
    parser.add_argument(
        "--model_id", required=True, help="Model ID to use in examples (e.g., google-bert/bert-base-uncased)"
    )
    parser.add_argument(
        "--model_category",
        required=True,
        choices=["TEXT", "VISION", "AUDIO", "MULTIMODAL"],
        help="Model category for documentation organization (e.g., TEXT, VISION)",
    )
