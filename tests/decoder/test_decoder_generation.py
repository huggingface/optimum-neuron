...
def test_continuous_batching_two_requests(model_and_tokenizer):
    ...
    next_tokens = get_next_tokens(input_ids, position_ids, first_seq_ids, sampling_params)
    first_generated_tokens, first_position_ids = update_decode_tensors(next_tokens, position_ids)
    # Decode a few tokens
    FIRST_DECODE_TOKENS = 5
    for _ in range(FIRST_DECODE_TOKENS):
        next_tokens = get_next_tokens(next_tokens, first_position_ids, first_seq_ids, sampling_params)
        first_generated_tokens, first_position_ids = update_decode_tensors(
            next_tokens, first_position_ids, first_generated_tokens
        )
    # Prefill a second input at index 2
    second_seq_ids = torch.tensor([2])
    next_tokens = get_next_tokens(input_ids, position_ids, second_seq_ids, sampling_params)
    second_generated_tokens, second_position_ids = update_decode_tensors(next_tokens, position_ids)
    ...
    next_tokens = generated_tokens[:, -1:]
    ...
    for _ in range(SECOND_DECODE_TOKENS):
        next_tokens = get_next_tokens(next_tokens, position_ids, two_requests_seq_ids, two_requests_sampling_params)
        generated_tokens, position_ids = update_decode_tensors(next_tokens, position_ids, generated_tokens)
