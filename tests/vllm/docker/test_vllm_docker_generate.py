...
@pytest.mark.asyncio
async def test_model_greedy_generation(multi_model_vllm_service):
...
@pytest.mark.asyncio
async def test_model_sampling_parameters(base_vllm_service):
    prompt = "What is Deep Learning?"
    max_output_tokens = 17
    # Greedy bounded without input
    greedy_tokens, greedy_text = await base_vllm_service.client.greedy(prompt, max_output_tokens=max_output_tokens)

    assert greedy_tokens == max_output_tokens

    # Sampling
    sample_tokens, sample_text = await base_vllm_service.client.sample(
        prompt,
    ...
    greedy_tokens_with_stop, greedy_text_with_stop = await base_vllm_service.client.greedy(
