...
@pytest.fixture
async def vllm_docker_service_embedding(vllm_docker_launcher, neuron_llm_config):
    """Fixture for vLLM docker service with embedding task."""
    service_name = neuron_llm_config["name"]
    model_name_or_path = neuron_llm_config["neuron_model_path"]

    with vllm_docker_launcher(service_name, model_name_or_path, task="embed") as vllm_service:
...
async def test_vllm_docker_service_embedding(vllm_docker_service_embedding, neuron_llm_config):
    """Test vLLM docker service embedding generation."""
    ...
    # Get embeddings from the vLLM docker service via OpenAI API
    client = vllm_docker_service_embedding.client

    # Create embeddings for each input text
    response = await client.embeddings.create(
        input=input_texts,
        model=vllm_docker_service_embedding.client.model_name,
    )
