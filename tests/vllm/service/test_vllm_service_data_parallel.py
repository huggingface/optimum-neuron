...
"""Tests for data-parallel serving via optimum-cli neuron serve."""
...
@pytest.fixture
async def vllm_dp_service(vllm_launcher, neuron_llm_config):
    """Launch a data-parallel vLLM service with 2 replicas."""
...
async def test_vllm_service_data_parallel_greedy(vllm_dp_service):
    """Verify greedy generation works with data-parallel serving."""
    prompt = "What is Deep Learning?"
    max_output_tokens = 17
    greedy_tokens, greedy_text = await vllm_dp_service.client.greedy(prompt, max_output_tokens=max_output_tokens)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("neuron_llm_config", ["qwen3-tp1-4x1024"], indirect=True)
async def test_vllm_service_data_parallel_sampling(vllm_dp_service):
    """Verify sampling works with data-parallel serving."""
    prompt = "What is Deep Learning?"
    max_output_tokens = 17
    ...
    sample_tokens, sample_text = await vllm_dp_service.client.sample(
        prompt,
