...
#         pipe = pipeline("automatic-speech-recognition", model=neuron_model, feature_extractor=processor.feature_extractor, tokenizer=processor.tokenizer)
#         dataset = load_dataset("hf-internal-testing/librispeech_asr_demo", "clean", split="validation")
#         dataset = dataset.sort("id")
...
#         pipe = pipeline("audio-classification", model=neuron_model, feature_extractor=processor.feature_extractor, tokenizer=processor.tokenizer)
#         dataset = load_dataset("hf-internal-testing/librispeech_asr_demo", "clean", split="validation")
#         dataset = dataset.sort("id")
#         _ = pipe(dataset[0]["audio"]["array"])
...
class NeuronModelForAudioFrameClassificationIntegrationTest(NeuronModelTestMixin):
    ...
    def _load_neuron_model_and_processor(self, model_arch, suffix):
        model_id = self.ARCH_MODEL_MAP[model_arch] if model_arch in self.ARCH_MODEL_MAP else MODEL_NAMES[model_arch]
        neuron_model = NeuronModelForAudioFrameClassification.from_pretrained(self.neuron_model_dirs[model_arch + suffix])
