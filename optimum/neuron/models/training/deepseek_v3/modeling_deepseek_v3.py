...
class DeepseekV3MLP(nn.Module, CustomModule):
    def __init__(self, config, trn_config: TrainingNeuronConfig, hidden_size: int | None = None, intermediate_size: int | None = None):
        super().__init__()
        self.config = config
...
class DeepseekV3DecoderLayer(LlamaDecoderLayer, nn.Module):
    def __init__(self, config: DeepseekV3Config, trn_config: TrainingNeuronConfig, layer_idx: int):
        ...
        self.self_attn = DeepseekV3Attention(config=config, trn_config=trn_config, layer_idx=layer_idx)

        if layer_idx >= config.first_k_dense_replace:
            self.mlp = DeepseekV3MoE(config, trn_config)
        else:
            self.mlp = DeepseekV3MLP(config, trn_config)
...
class DeepseekV3Model(NeuronModelMixin, DeepseekV3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"model\.layers\.61.*"]

    def __init__(self, config: DeepseekV3Config, trn_config: TrainingNeuronConfig):
        ...
        self.layers = nn.ModuleList(
            [DeepseekV3DecoderLayer(config, trn_config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class DeepseekV3ForCausalLM(NeuronModelMixin, DeepseekV3PreTrainedModel):
    ...
    def __init__(self, config, trn_config: TrainingNeuronConfig):
        DeepseekV3PreTrainedModel.__init__(self, config)
        self.model = DeepseekV3Model(config, trn_config)
        self.trn_config = trn_config
