...
class NxDSmolVLMDecoderModel(NxDLlamaModel):
    ...
    def forward(
        ...
        if self._is_context_encoding(input_ids) and image_embeds is not None and image_token_mask is not None:
...
class SmolVLMNxDModelForImageTextToText(NxDModelForImageTextToText):
    ...
    Manages indexed compiled traced bundles:
        ...
        * ``model_0.pt`` — vision encoder bundle (SigLIP vision encoder +
            HF ``Idefics3Connector``).
        * ``model_1.pt`` and above — decoder graph bundle(s) for context
