...
### Compiled artifacts (indexed)
All compiled bundles are saved with the indexed naming scheme defined in
[backend/pretrained_model.py](../backend/pretrained_model.py) and must form a
contiguous sequence starting from 0 (glob `model_*.pt`):

- `model_0.pt`: vision encoder bundle — SigLIP vision encoder + HF `Idefics3Connector` traced with ModelBuilder.
- `model_1.pt`: text decoder graph bundle — context/chunked-prefill + token generation.
...
`load_weights()` is called to restore the shard checkpoints for all bundles.
