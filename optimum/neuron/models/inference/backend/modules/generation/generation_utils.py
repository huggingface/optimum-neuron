...
class NxDGenerationMixin(GenerationMixin, ABC):
    ...
    def _assisted_decoding(
        ...
        **kwargs,
        ...
        # Increment attention mask and position ids for the accepted token
        attention_mask = increase_attention_mask(attention_mask, 1)
        position_ids = increase_position_ids(position_ids, 1)
        ...
        while input_ids.shape[1] < max_len:
            ...
            # The last accepted token must be added because it has never been processed by the original model yet.
            # This last candidate token is purely speculative as even the assistant model did not compute any logits for it yet.
            validation_tokens = torch.cat((input_ids[:, -1:], candidate_tokens[:, :-1]), dim=-1)
            # The attention mask and seq_ids are the same as before
            # The position ids are updated to cover the new tokens
