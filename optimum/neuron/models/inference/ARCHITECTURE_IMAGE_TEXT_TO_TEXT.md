...
┌──────────────────────────────────────────────────────┐
│  model_1.pt — Text Decoder (shared weights)           │
│                                                       │
│  ┌─────────────────────────┐  ┌─────────────────────┐ │
...
### Minimal refactor in `NxDDecoderModelForCausalLM`
...
- **Vision builder** → traced into `model_0.pt`
- **Context encoding builder** (6 tensors) → traced into `model_1.pt`
- **Token generation builder** (4 tensors, standard) → traced into `model_1.pt`

The context encoding and token generation builders feed into the **same**
`ModelBuilder` instance (shared weights). The vision builder feeds into a
separate `ModelBuilder` instance.
...
forward(input_ids, position_ids, seq_ids, pixel_values=None, ...)
   ...
   │  │                                                          │
   │  │  1. Run vision encoder (model_0.pt):                     │
   ...
   │  │                                                          │
   │  │  3. Run context encoding (model_1.pt, 6 tensors):       │
...
User code:
 ...
 │                                                                    │
 │    1. create_vision_graph_builders()                               │
 │       → NxDVisionEncoderBuilder(vision_encoder_cls, ...)           │
 │       → ModelBuilder.trace() → model_0.pt (vision weights)        │
 │                                                                    │
 │    2. create_graph_builders()                                      │
 │       → NxDDecoderBuilderForImageTextToText  (ctx enc, 6 tensors) │
 │       → NxDDecoderBuilderForCausalLM         (tok gen, 4 tensors) │
 │       → ModelBuilder.trace() → model_1.pt (text decoder weights)  │
 │                                                                    │
 │    3. SmolVLMNxDModelForImageTextToText.__init__()                 │
 │       → vision_encoder_wrapper (wraps model_0.pt)                 │
 │       → context_encoding_model (VLM wrapper, wraps model_1.pt)    │
 │       → token_generation_model (standard wrapper, wraps model_1.pt│
 │                                                                    │
 │    4. load_weights() for both bundles                              │
 │                                                                    │
 └────────────────────────────────────────────────────────────────────┘
 ...
 │    │     token_generation_model(4 tensors, model_1.pt)      │       │
...
### 10a. Remove dummy image tensors from token generation

**Current:** Token generation wrapper creates dummy `image_embeds` and
...
### 10b. Use `compute_input_embeddings()` hook instead of modifying the base forward

**Current:** `vlm_decoder.py` duplicates or heavily patches
...
### 10c. Rename classes to match HF task convention

**Current:** `NxDVLMModelForCausalLM`, `NxDVLMContextDecoderWrapper`, etc.
...
### 10d. Eliminate VLM-specific wrapper subclasses where possible

**Current:** `NxDVLMContextDecoderWrapper` and `NxDVLMTokenGenerationWrapper`
...
│           └── modeling_decoder.py               # (one-line refactor)
├── smolvlm/
