...
from ..backend.model_wrapper import NxDModelWrapper
from ..backend.pretrained_model import NxDPreTrainedModel
...
class NeuronCLIPAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.q_proj = ColumnParallelLinear(config.hidden_size, config.hidden_size, gather_output=False)
        self.k_proj = ColumnParallelLinear(config.hidden_size, config.hidden_size, gather_output=False)
        self.v_proj = ColumnParallelLinear(config.hidden_size, config.hidden_size, gather_output=False)
        self.out_proj = RowParallelLinear(config.hidden_size, config.hidden_size, input_is_parallel=True)
...
class NeuronCLIPMLP(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = ColumnParallelLinear(config.hidden_size, config.intermediate_size, gather_output=False)
        self.fc2 = RowParallelLinear(config.intermediate_size, config.hidden_size, input_is_parallel=True)
...
class NeuronCLIPEncoder(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([NeuronCLIPEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class NxDClipWrapper(NxDModelWrapper):
    """
...
class CLIPTextNxDModel(NxDPreTrainedModel):
    ...
    @staticmethod
    def convert_hf_to_neuron_state_dict(state_dict: dict, config: NxDNeuronConfig) -> dict:
        new_load = {
            key.replace("text_model.", "neuron_text_encoder."): state_dict[key].clone().detach().contiguous()
