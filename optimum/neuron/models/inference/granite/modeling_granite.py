...
class NeuronGraniteAttention(NeuronLlamaAttention):
    """
    The only difference with the LlamaAttentionBase is the custom scaling factor or the QK projections
    """

    def scaled_qk(self, Q, K, attention_mask):
        # Granite specific: use trained attention multiplier instead of legacy constant scale factor
        QK = torch.matmul(Q, K.transpose(2, 3)) * self.config.attention_multiplier
...
class NeuronGraniteDecoderLayer(nn.Module):
    ...
    def __init__(self, config: GraniteConfig, neuron_config: NxDNeuronConfig):
        ...
        self.self_attn = NeuronGraniteAttention(config, neuron_config)
