...
class OptimumNeuronModel(nn.Module):
    """Base class for optimum-neuron models used with vLLM."""

    auto_class: type[NeuronPreTrainedModel]

    def __init__(self, model: NeuronPreTrainedModel) -> None:
        super().__init__()
        self.model = model

    @classmethod
    def create(
        cls: type["OptimumNeuronModel"],
        ...
        if neuron_config is not None:
            ...
            neuron_config = cls.auto_class.get_neuron_config(
                model_name_or_path=model_name_or_path,
                batch_size=batch_size,
                sequence_length=sequence_length,
                tensor_parallel_size=tp_degree,
            )
            neuron_model = cls.auto_class.export(
                model_name_or_path,
        ...
        return cls(neuron_model)


class OptimumNeuronModelForCausalLM(OptimumNeuronModel):
