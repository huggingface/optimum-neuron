...
def read_memory_counter(base_path: Path, category: str) -> MemoryCounter:
    ...
    try:
        total = int(read_sysfs_value(counter_path / "total"))
    except (FileNotFoundError, PermissionError, ValueError):
        total = None
    try:
        present = int(read_sysfs_value(counter_path / "present"))
    except (FileNotFoundError, PermissionError, ValueError):
        present = None
    try:
        peak = int(read_sysfs_value(counter_path / "peak"))
    except (FileNotFoundError, PermissionError, ValueError):
        peak = None

    return MemoryCounter(total=total, present=present, peak=peak)
