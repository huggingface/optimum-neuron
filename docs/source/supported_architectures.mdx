...
**Requirements:**
- Model must be compatible with the Neuron SDK (if small enough to fit in 16GB, training is supported for any architecture that can be compiled)
- **Memory constraint:** Each accelerator has 16GB memory for model weights, gradients, optimizer states, and activations
