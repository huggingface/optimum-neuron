...
# Fine-tune Llama 3 70B on one AWS Trainium instance with LoRA
...
# model_id = "meta-llama/Meta-Llama-3-70B" # gated
model_id = "NousResearch/Meta-Llama-3-70B" # ungated
...
After we processed the datasets we can save them to disk. You could also save it to S3 or the Hugging Face Hub for later use.
...
## 3. Fine-tune Llama using LoRA on AWS Trainium with the `NeuronTrainer`
...
Since Llama-3 70B is a big model it will not fit on a single Neuron device, even with distributed training. To actually fine-tune a 70B model using only one Trainium instance we need to use both LoRA and distributed training. In Optimum Neuron we support:
