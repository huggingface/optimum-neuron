...
This tutorial shows how to fine-tune the Llama 3 model on AWS Trainium accelerators using optimum-neuron.

**This is based on the [Llama 3 fine-tuning example script](https://github.com/huggingface/optimum-neuron/tree/main/examples/training/llama).**
...
## 3. 🎯 Fine-tune Llama 3 with NeuronSFTTrainer and PEFT
...
Since Llama 3 8B doesn't fit on a single accelerator, we use distributed training techniques:
- Data Parallel (DDP)
- Tensor Parallelism  

Model loading and LoRA configuration work similarly to other accelerators.

Combining all the pieces together, and assuming the dataset has already been loaded, we can write the following code to fine-tune Llama 3 on AWS Trainium:

```python
model_id = "meta-llama/Meta-Llama-3-8B"

# Define the training arguments
output_dir = "Meta-Llama-3-8B-finetuned"
...
```bash
optimum-cli neuron consolidate Meta-Llama-3-8B-finetuned Meta-Llama-3-8B-finetuned/adapter_default
```
...
ADAPTER_PATH = "Meta-Llama-3-8B-finetuned/adapter_default"
MERGED_MODEL_PATH = "Meta-Llama-3-8B-dolly"
...
You have successfully created a fine-tuned model from Llama 3!
...
MERGED_MODEL_PATH = "Meta-Llama-3-8B-dolly"
HUB_MODEL_NAME = "your-username/llama3-8b-dolly"
...
🎉 **Your fine-tuned Llama 3 model is now available on the Hub for others to use!**
