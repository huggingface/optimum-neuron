...
## 3. 🎯 Fine-tune BERT with Trainer

For training on AWS Trainium, we use the [`NeuronTrainer`](https://huggingface.co/docs/optimum-neuron/training_api/trainer) and [`NeuronTrainingArguments`](https://huggingface.co/docs/optimum-neuron/training_api/trainer) classes from Optimum Neuron.
...
📝 **Complete script available:** All steps above are combined in a ready-to-use script [finetune_bert.py](https://github.com/huggingface/optimum-neuron/blob/main/examples/training/bert/finetune_bert.py).
...
torchrun --nproc_per_node=2 finetune_bert.py \
  --model_id bert-base-uncased \
  --learning_rate 5e-5 \
  --batch_size 8 \
  --epochs 3 \
...
🔧 **Single command execution:** The complete bash training script [finetune_bert.sh](https://github.com/huggingface/optimum-neuron/blob/main/examples/training/bert/finetune_bert.sh) is available:

#### Precompilation (Optional)

For faster training, you can precompile the model graphs:
...
🎉 **Your fine-tuned BERT model is now available on the Hub for others to use!**
