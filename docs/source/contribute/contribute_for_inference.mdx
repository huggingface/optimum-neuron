...
> **_NOTE:_** ❗This section applies to models traced with `torch_neuronx`. Decoder models used for text generation rely on the NxD backend instead: to add support for such a model, follow the guide in [optimum/neuron/models/inference](https://github.com/huggingface/optimum-neuron/tree/main/optimum/neuron/models/inference).
