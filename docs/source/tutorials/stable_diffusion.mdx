...
We recommend using `inf2.8xlarge` or larger instance for the compilation. You will also be able to compile the model with Optimum CLI on a CPU-only instance (needs ~35 GB memory), and then run the pre-compiled models on `inf2.xlarge` to reduce the expenses. In this case, don't forget to disable validation of inference by adding the `--disable-validation` argument.
...
We recommend using `inf2.8xlarge` or larger instance for the compilation. You will also be able to compile the model with Optimum CLI on a CPU-only instance (needs ~92 GB memory), and then run the pre-compiled models on `inf2.xlarge` to reduce the expenses. In this case, don't forget to disable validation of inference by adding the `--disable-validation` argument.
...
num_images_per_prompt = 2  # Multiple of 2 to leverage both cores on the Neuron device
...
pipe = NeuronLatentConsistencyModelPipeline.from_pretrained("lcm_sdxl_neuron/")
