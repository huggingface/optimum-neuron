...
Before installing `optimum-neuron` make sure that you have installed the Neuron driver and tools, check out [more detailed guide here](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/setup/pytorch/index.html).
...
```bash
uv pip install optimum-neuron[neuronx] --extra-index-url https://pip.repos.neuron.amazonaws.com --index-strategy unsafe-best-match
```
