...
Since the example scripts use the `NeuronTrainer`, ZeRO-1 is enabled by default when using them (`--zero_1`). You can disable it by passing `--zero_1 false` to your command line.
...
accelerator = NeuronAccelerator(
    ...
    zero_1=True,  # Enable ZeRO-1
    mixed_precision_config="FULL_BF16",
)
...
All you need to do is specify the sharded checkpoints directory and the output directory that will contain the consolidated checkpoints (it defaults to the checkpoints directory), and the command takes care of the rest.
