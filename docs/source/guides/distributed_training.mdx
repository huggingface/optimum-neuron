...
Whether you use to [`~optimum.neuron.NeuronTrainer`] or decide to have your own training script that uses the [`~optimum.neuron.NeuronAccelerator`], it is very easy to enable the ZeRO-1 optomization.
...
Just as for ZeRO-1, it is possible to use Tensor Parallelism both with the [`~optimum.neuron.NeuronTrainer`] and the [`~optimum.neuron.NeuronAccelerator`].
...
To alleviate that `optimum-neuron` provides a context-manager [`~optimum.neuron.distributed.lazy_load_for_parallelism`] that loads the model lazily, only the sharded model will be materialized in each worker.
