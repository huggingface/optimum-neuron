...
As explained in the previous paragraph, you will need only a few modifications to your Transformers code to export and run NLP models:
...
- `batch_size` is the number of input sequences that the model will accept,
...
bigger models need to be split on multiple cores,
- `auto_cast_type` specifies the format to encode the weights. It can be one of `f32` (`float32`), `f16` (`float16`) or `bf16` (`bfloat16`).

As explained before, these parameters can only be configured during export. This means in particular that
the `batch_size` of the inputs during inference cannot be changed once the model has been exported.

If not specified, these parameters will take default values.
