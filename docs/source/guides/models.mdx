...
- `input_shapes = { batch_size, sequence_length }` correspond to the static shape of the model input and Key, Value cache.
...
+compiler_args = {"num_cores": 1, "auto_cast_type": 'f32'}
...
- the maximum nember of tokens (input + generated) cannot exceed the `sequence_length` used during export.
