...
- What if real input sizes are smaller than compilation input shapes?

No worries, `NeuronModelForXXX` class will pad your inputs to an eligible shape. 

Besides you can set `dynamic_batch_size=True` in the `from_pretrained` method to enable dynamic batching, which means that your inputs can have variable batch size.
