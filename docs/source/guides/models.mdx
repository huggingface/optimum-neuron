...
## Export and Inference of Generative Models

only add decoder specific items, ex: mandatory shape(bs_size), dependency(transformers-neuronx?)

if the export cannot done with CLI, maybe add a snippet on how to export all necessary artifacts (and upload it to the hub when it's available) here.


## Inference of Stable Diffusion Models

You can also accelerate the inference of stable diffusion on neuronx devices (inf / trn1). There are three components which need to be exported to `.neuron` format to boost the performance:
...
>>> input_shapes = {"batch_size": 2, "sequence_length": 18, "num_channels": 4, "height": 64, "width": 64}
...
-- TODO: add an sd generated image example. 
