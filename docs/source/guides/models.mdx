...
`NeuronModelForXXX` classes help to models from the [Hugging Face Hub](hf.co/models) to a serialized format optimized for neuron devices. You will
then be able to load the model and run inference with the acceleration powered by AWS Neuron devices.
...
If you already use Transformers, you will be able to reuse your code just by replacing the model classes:
...
As shown above, when you use `NeuronModelForXXX` for the first time, you will need to set `export=True` to compile your model from PyTorch to a neuron-compatible model.
`input_shapes` are mandatory static shape information that you need to send to the neuron compiler. Want to know what shapes are mandatory for your model? Check it out 
...
What if the real input sizes are smaller than compilation input shapes?
