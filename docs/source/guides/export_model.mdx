...
In the last section, you can see some input shape options to pass for exporting static neuron model, meaning that exact shape inputs should be used during the inference as given during compilation. If you are going to use variable-size inputs, you can pad your inputs to the shape used for compilation as a workaround.
...
```bash
optimum-cli export neuron --model local_path --task question-answering --batch_size 1 --sequence_length 16 distilbert_base_uncased_squad_neuron/
```
...
You will also be able to export your models to Neuron format directly with the export function, and load it for inference. Here is an example:
...
neuron_config = DistilBertNeuronConfig(
    config=model.config, task="text-classification", batch_size=1, sequence_length=18,
)
...
neuron_model = torch.jit.load(output_path)
example = tokenizer.encode_plus(
    sequence, max_length=neuron_config.sequence_length, padding="max_length", return_tensors="pt"
)

logits = neuron_model(*tuple(example.values()))
```
