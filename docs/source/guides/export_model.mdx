...
If you are using a `NeuronModelForXXX` class to export the model on a CPU-only instance, set the target instance type through the `instance_type` argument, For example:

```python
model = NeuronModelForXXX.from_pretrained(
    ...,
    export=True,
    instance_type="inf2",
    ...
