...
```bash
optimum-cli export neuron --model bert-base-uncased bert_neuron/
```
...
AWS provides two generations of the Inferentia accelerator built for machine learning inference: [INF2 (NeuronCore-v2)](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/general/arch/neuron-hardware/inf2-arch.html) and [INF1 (NeuronCore-v1)](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/general/arch/neuron-hardware/inf1-arch.html#aws-inf1-arch), to achieve higher throughput and lower latency with lower cost.  

In production environments, to deploy 🤗 [Transformers](https://huggingface.co/docs/transformers/index) models on Neuron devices, you need to compile your models and export them to a serialized format before inference on Neuron devices. Through Ahead-Of-Time (AOT) compilation with Neuron Compiler([neuronx-cc](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/release-notes/compiler/neuronx-cc/index.html) or [neuron-cc)](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/release-notes/compiler/neuron-cc/neuron-cc.html)), your models will be converted to serialized and optimized [TorchScript modules](https://pytorch.org/docs/stable/generated/torch.jit.ScriptModule.html).
...
The resulting `model.neuron` file, can be loaded and run on Neuron devices. We plan to add support for inference using the same API as Transformers. Here is an example (not in `Optimum-neuron` repo yet, stay tuned!)

```python
>>> from transformers import AutoTokenizer
>>> from optimum.neuron import NeuronModelForQuestionAnswering

>>> tokenizer = AutoTokenizer.from_pretrained("distilbert_base_uncased_squad_neuron")  # doctest: +SKIP
>>> model = NeuronModelForQuestionAnswering.from_pretrained("distilbert_base_uncased_squad_neuron")  # doctest: +SKIP

>>> inputs = tokenizer("What shall I do?", "Running DistilBERT on INF2!", return_tensors="pt")  # doctest: +SKIP
