...
| Other `NeuronModelForXXX` | ✅ Full | Inference | Traced artifacts, cached as a single entry |
| Diffusion pipelines | ✅ Full | Inference | Traced artifacts, cached as a multi-model entry |

<Tip>

Caching can be turned off for a single export with the `--disable_neuron_cache` option of `optimum-cli export neuron`, or by passing `disable_neuron_cache=True` when exporting a model with `from_pretrained`.
...
# Show the local cache status (entry counts, disk usage, compiler versions)
optimum-cli neuron cache status

# Remove failed compilations and stale locks from the local cache
optimum-cli neuron cache cleanup
```
