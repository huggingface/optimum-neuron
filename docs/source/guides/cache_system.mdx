...
- **Runtime caching**: Compiled graphs are automatically cached as they're generated during training
- **Auto upload**: New compilations are uploaded to your cache repository during training
- **Private repos**: Supports team-specific caches for collaboration
- **Context managers**: Uses `hub_neuronx_cache` to patch the compiler transparently

### Inference Cache (Inferentia) 
- **Pre-compiled lookup**: Search a registry of cached model configurations before compilation
- **Public cache only**: Read-only access to the shared public cache (`aws-neuron/optimum-neuron-cache`)
- **Registry search**: Use `optimum-cli neuron cache lookup` to find compatible models
- **NeuronModelForCausalLM integration**: Automatically downloads cached models if available

### Traced Model Cache (Both platforms)
- **Manual workflow**: You explicitly cache compiled artifacts after tracing models
- **Ahead-of-time (AOT) compilation**: Pre-compile models once, deploy everywhere
- **Cross-platform**: Works on both Trainium and Inferentia
- **Production ready**: Good for deployment scenarios where compilation time must be eliminated

## Best Practices

### Configuration Management
✅ **Use consistent configurations** across your team to maximize cache hits  
✅ **Standardize parameters** like batch size, sequence length, and precision  
✅ **Document your configurations** so team members use compatible settings  
✅ **Pin compiler versions** in CI/CD to ensure consistent cache behavior  

### Cache Operations
✅ **Create private cache** for your organization to store your compilations  
✅ **Check cache hits** in logs (look for "Fetched cached" messages)  
✅ **Monitor cache repository size** as NEFF files can be large (GBs)  
✅ **Use lookup before compiling** to find compatible cached models  

### Common Pitfalls
❌ **Don't** frequently change compilation parameters without good reason  
❌ **Don't** skip authentication setup (`huggingface-cli login`)  
❌ **Don't** ignore "recompiling" warnings - investigate why cache missed  
❌ **Don't** assume cache hits - always verify with logs or lookup commands
