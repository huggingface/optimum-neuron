...
Before training a `transformers` or `diffusers` model or loading a NeuronModelForXXX on Neuron platforms, it needs to be exported to neuron format
with [`torch-neuronx`](https://github.com/aws-neuron/aws-neuron-samples/tree/master/torch-neuronx).

When exporting a model, [`torch-neuronx`](https://github.com/aws-neuron/aws-neuron-samples/tree/master/torch-neuronx) will either:
...
- 2nd approach: record one graph via tracing (TorchScript), compile it with the neuronx compiler to a NEFF binary file, and serialize as a TorchScript module (eg. inference of encoders, stable diffusion and seq2seq models).
...
To avoid recompiling all NEFF files, for models adopting the first approach, every time a model is loaded on a NeuronX host, [`torch-neuronx`](https://github.com/aws-neuron/aws-neuron-samples/tree/master/torch-neuronx)
 stores NEFF files in a local directory, usually `/var/tmp/neuron-compile-cache`. And for models adopting the second approach, the compiled Neuron models with checkpoint artifacts (eg. config, tokenizer) will be stored by `optimum-neuron` after the the first time compilation as well.
...
It is important to understand that the cache of training / models applying `transformers_neuronx` operates on NEFF binaries, and not on the model itself.
