...
- to deploy vanilla Hugging Face hub models that are not cached on the Hugging Face hub, it
is still possible to use the default `neuron` platform from the vLLM package.
