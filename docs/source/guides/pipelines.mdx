...
NeuronModels currently require static `input_shapes` to run inference. If you are not providing input shapes when providing the `export=True` parameter, the default input shapes will be used. 
Below is an example on how to specifc the input shapes for the sequence length and batch size. 
