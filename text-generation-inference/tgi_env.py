...
def parse_cmdline(argv: List[str] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--max-total-tokens", type=int,
                        default=os.getenv("MAX_TOTAL_TOKENS", os.getenv("HF_SEQUENCE_LENGTH", 0)))
    parser.add_argument("--max-batch-size", type=int,
                        default=os.getenv("MAX_BATCH_SIZE", os.getenv("HF_BATCH_SIZE", 0)))
    ...
    if not args.model_id:
        raise Exception("No model id provided ! Either specify it using --model-id cmdline "
                        "or MODEL_ID env var")
...
def lookup_compatible_cached_model(model_id: str, revision: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    if not entry:
        logger.debug("No compatible cached entry found for model %s, env %s, available cores %s, "
                     "neuronxcc version %s",
                     model_id, get_env_dict(),
                     available_cores, neuronxcc_version)
    else:
        logger.info("Compatible neuron cached model found %s", entry)
    return entry


def check_env_and_neuron_config_compatibility(revision: Optional[str],
                                              neuron_config: Dict[str, Any]) -> bool:

    logger.debug("Checking the provided neuron config %s is compatible with the local setup and "
                 "provided environment", neuron_config)

    checkpoint_revision = neuron_config['checkpoint_revision']
    if revision and revision != checkpoint_revision:
        logger.debug("Specified revision does not match with the model one (%s != %s)",
    ...
    if neuron_config["compiler_version"] != neuronxcc_version:
        logger.debug("Compiler version conflict, the local one "
                     "(%s) differs from the one used to compile the model (%s)",
                     neuronxcc_version, neuron_config["compiler_version"])
        return False
    ...
    for env_var, config_key in env_config_peering:
        ...
        if env_value != neuron_config_value:
            logger.debug("The provided env var '%s' and the neuron config '%s' param differ (%s != %s)",
                         env_var, config_key, env_value, neuron_config_value)
            return False
    ...
    if os.getenv("MAX_INPUT_LENGTH"):
        ...
        if max_input_length >= sequence_length:
            logger.debug("Specified max input length is not compatible with config sequence length "
                         "( %s >= %s)", max_input_length, sequence_length)
            return False
...
def main():
    ...
    if neuron_config is not None:
        compatible = check_env_and_neuron_config_compatibility(args.revision, neuron_config)
        if not compatible:
            env_dict = get_env_dict()
            msg = ("Invalid neuron config and env. Config {}, env {}, available cores {}, "
                   "neuronxcc version {}").format(neuron_config, env_dict, available_cores,
                                                  neuronxcc_version)
            logger.error(msg)
            raise Exception(msg)
    else:
    ...
    if not neuron_config:
        msg = ("No compatible neuron config found. Provided env {}, "
               "available cores {}, neuronxcc version {}").format(get_env_dict(), available_cores,
                                                                  neuronxcc_version)
        logger.error(msg)
        raise Exception(msg)
